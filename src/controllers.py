...
class CEM:
    ...
    def get_action(self, current_observation, dynamics_model, params, horizon, key):
        ...
        def estimate_return(current_observation, action_sequence):
        ...
        def update_action_sequence(action_mean, key, self, current_observation):
            ...
            total_reward = batch_estimate_return(current_observation, action_sequences)
        ...
        #     # sample multiple action sequences from the action sequence distribution
        #     action_sequences = np.expand_dims(action_mean, 2) + np.expand_dims(np.sqrt(action_variance), 2) * random.normal(next(subkeys), (horizon, self.n_actions, self.n_sequences))
        ...
        #     # update the mean and variance of the action sequence distribution
        #     action_mean = self.alpha * np.sum(action_sequences * weights, 2) + (1 - self.alpha) * action_mean
